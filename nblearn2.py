...
if __name__ == '__main__':
    ...
    fivePercentSpamLen = int(len(spamFiles) * 0.1)
    #print(fivePercentSpamLen)
    hamFiles = findAllHamFiles(direcPath)

    fivePercentHamLen = int(len(hamFiles) * 0.1)
    #print(fivePercentHamLen)

   # totNumber = len(spamFiles) + len(hamFiles)
    totNumber = fivePercentSpamLen + fivePercentHamLen
    probOfSpam = fivePercentSpamLen / totNumber
    spamFilesLen = len(spamFiles)
    hamFilesLen = len(hamFiles)
    spamFivePercentFiles = spamFiles[3000:fivePercentSpamLen+3000]
    hamFivePercentFiles = hamFiles[1000:fivePercentHamLen+1000]
    #print(len(spamFivePercentFiles), len(hamFivePercentFiles))
    noOfWordsInSpam = 0
    noOfWordsInHam = 0
    classDictList = tokenizeWordsFromList(spamFivePercentFiles, hamFivePercentFiles)
